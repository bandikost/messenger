...
const Message = ({ message, time, chat, id }) => {
  ...
  const handleContextMenu = (e) => {
    e.preventDefault();
    setSelectedMessageId(message.id); // Assuming the unique ID of each message is stored in the 'id' property
  ...
  return (
    ...
    {message && (
      <React.Fragment>
        ...
        {isMenuOpen && selectedMessageId === message.id && (
  <div
  ...
  >
    <ul className="menuContent" style={{ padding: "0px 10px", marginLeft: "-90px", marginTop: "10px" }}>
      ...
      <li onClick={handleEdit} style={{ marginLeft: "35px" }}>
        <img src="" width={"20px"} style={{ marginRight: "0px", marginLeft: "-50px" }} alt="" />
