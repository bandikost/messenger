...
const Input = () => {
  ...
  return ( 
    ...
    <input
      ...
      <div className="send">
        <img src={Attach} alt="" style={{ display: showImage > 0 ? "flex" : "none" }} />
        <input 
          type="file"
          style={{ display: "none" }}
          id="file"
        ...
        <label className="attach" htmlFor="file" style={{ display: showImage > 0 ? "none" : "flex" }}>
          <img style={{position: "relative", top: "3px"}} src={Img} alt="" />
        </label>

        <button onClick={handleClick} disabled={isSendButtonDisabled}>
          {isSmallScreen ? <img src={send} width={"12px"}  alt="Icon" style={{display: "flex", position: "relative"}} /> : 'Отправить'}
        </button>
