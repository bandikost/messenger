    ...
    <div className="chat" onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
      <div className="chatInfo">  
        <div className="chatinfo-container">
          ...
          <span className={showImage ? 'show' : ''} >
            <img className={showImage ? 'show-img' : ''} src={data.user?.photoURL} alt="" width={"50px"} height={"50px"} style={{borderRadius: showImage > 0 ? "0%" : '50%', objectFit: "fill" }}  />
              <span>
          ...
          {isOpen && (
            <div className='more'> 
                <a className='' href="#" onClick={() => setIsOpens(!isOpens)} style={{display: "flex"}}>
                  ...
                  <img src={back} width={"18px"} alt="" style={{marginTop: "2px", marginLeft: "20px", rotate: "180deg", width: "8px"}} />
                </a>                  
                 
              <hr style={{height: "7px", backgroundColor: "#ddddf7", color: "#ddddf7", border: "none"}}/>
                <li>
                  <img src={search} style={{marginLeft: "-5px"}} alt="" width={"28px"}/>
                  <span>Поиск</span>
                </li>
                <li>
                  <img src={add} style={{marginLeft: "-2px"}} width={"26px"} alt="" />
                  <span>Создать беседу</span>
                </li>
              <a href='#' className='' onClick={() => setisOpenClear(!isOpenClear) }>
                <img src={deleteChat} width={"20px"} alt=""  />
                <span style={{marginLeft: "10px"}}>Очистить диалог</span>
              </a>
              {isOpenClear && (
